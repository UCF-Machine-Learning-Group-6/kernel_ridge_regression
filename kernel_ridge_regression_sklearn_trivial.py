...
for train_index, test_index in folds.split(dataset_cv):
    x_train_cv = np.asarray([[row['pH']] for index, row in dataset_cv.iloc[train_index].iterrows()])
    x_test_cv = np.asarray([[row['pH']] for index, row in dataset_cv.iloc[train_index].iterrows()])
...
x_test_main = np.asarray([[row['pH']] for index, row in dataset_main.iterrows()])
