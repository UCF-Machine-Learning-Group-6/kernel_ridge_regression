...
for train_index, test_index in folds.split(dataset_cv):
    x_train_cv = np.asarray([[row['density']] for index, row in dataset_cv.iloc[train_index].iterrows()])
    x_test_cv = np.asarray([[row['density']] for index, row in dataset_cv.iloc[train_index].iterrows()])
    y_train_cv = np.asarray([[row['alcohol']] for index, row in dataset_cv.iloc[train_index].iterrows()])
    y_test_cv = np.asarray([[row['alcohol']] for index, row in dataset_cv.iloc[train_index].iterrows()])

x_test_main = np.asarray([[row['density']] for index, row in dataset_main.iterrows()])
y_test_main = np.asarray([[row['alcohol']] for index, row in dataset_main.iterrows()])
...
print('Accuracy Score of the Model : ' + str(acc_score))
plt.figure('Linear Kernel ~ SKLearn ~ Trivial')
...
acc_score = r2_score(y_test_main, y_krr) * 100
print('Accuracy Score of the Model : ' + str(acc_score))
plt.figure('Polynomial Kernel ~ SKLearn ~ Trivial')
plt.plot([x for x in range(0, len(y_test_main))], y_test_main, 'b.')
plt.plot([x for x in range(0, len(y_test_main))], y_krr, 'r-')
...
acc_score = r2_score(y_test_main, y_krr) * 100
print('Accuracy Score of the Model : ' + str(acc_score))
plt.figure('Gaussian Kernel ~ SKLearn ~ Trivial')
plt.plot([x for x in range(0, len(y_test_main))], y_test_main, 'b.')
plt.plot([x for x in range(0, len(y_test_main))], y_krr, 'r-')
