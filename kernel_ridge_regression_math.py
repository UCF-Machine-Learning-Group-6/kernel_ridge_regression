...
def train_test_split_cv(dataset):
    for train_index, test_index in folds.split(dataset):
        x_train = np.asarray([[row['fixed.acidity'], row['volatile.acidity'], row['citric.acid'], 
                               row['residual.sugar'], row['chlorides'], row['free.sulfur.dioxide'], 
                               row['total.sulfur.dioxide'], row['density'], row['pH'], row['sulphates'], 
                               row['alcohol']] for index, row in dataset.iloc[train_index].iterrows()])        
        x_test = np.asarray([[row['fixed.acidity'], row['volatile.acidity'], row['citric.acid'], 
                              row['residual.sugar'], row['chlorides'], row['free.sulfur.dioxide'], 
                              row['total.sulfur.dioxide'], row['density'], row['pH'], row['sulphates'], 
                              row['alcohol']] for index, row in dataset.iloc[test_index].iterrows()])
        y_train = np.asarray([[row['quality']] for index, row in dataset.iloc[train_index].iterrows()])
        y_test = np.asarray([[row['quality']] for index, row in dataset.iloc[test_index].iterrows()])
...
def test_sets_main(dataset):
    x_test = np.asarray([[row['fixed.acidity'], row['volatile.acidity'], row['citric.acid'], 
                          row['residual.sugar'], row['chlorides'], row['free.sulfur.dioxide'], 
                          row['total.sulfur.dioxide'], row['density'], row['pH'], row['sulphates'], 
                          row['alcohol']] for index, row in dataset.iterrows()])
    y_test = np.asarray([[row['quality']] for index, row in dataset.iterrows()])
...
if __name__ == "__main__":
    ...
    plt.figure('Polynomial Kernel ~ Math')
    plt.plot([x for x in range(0, len(y_test))], y_test, 'b.')
    ...
    plt.figure('Gaussian Kernel ~ Math')
    plt.plot([x for x in range(0, len(y_test))], y_test, 'b.')
